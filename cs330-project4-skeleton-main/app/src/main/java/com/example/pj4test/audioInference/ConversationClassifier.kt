...
import com.example.pj4test.fragment.counter
...
class ConversationClassifier {
    ...
    fun startInferencing() {
        if (task == null) {
            task = Timer().scheduleAtFixedRate(0, REFRESH_INTERVAL_MS) {
                // Audio recognition begins in 10 seconds when the user is not reading the book
                if (counter.get_book_count() > 300) {
                    val score = inference()
    ...
    companion object {
        ...
        // Audio recognition time interval
